...
def connect_database():
    ...
    ebhs_database = pymysql.connect(host="127.0.0.1", user="root", password="1234567899", database="Ebbinghaus",
                                    charset="utf8")
...
def review():
    ...
    print("#################################################")
    print("#\t\t\t\t\t\t#")
    content_input = input("#\t复习的内容 \tor\t 退出？\t\t#\n#\t\t\t\t\t\t#\n#################################################\n:")
    connect_database()
    ebhs_table_point.execute('select * from Ebbinghaus')
    all = ebhs_table_point.fetchall()
    length = len(all)
