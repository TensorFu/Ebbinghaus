...
# 复习一个章节并且能够在复习一次章节之后在stage上加1，上限是8.
def review():
    ...
    for key in dic_date:
        if review_date == str(dic_date[key][0]):
            ...
            if remake_stage_int >= 8:
                dic_date[key][1] = '8'
            dic_date[time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())] = dic_date.pop(key)
            break
    else:
        print("\n请核实再输入\n")
...
def renew_content():
    ...
    if input("是否继续\n") == "是":
        remake_list = ["时间\t" + "\t" + "\t", "内容\t" + "\t" + "\t", "阶段\n"]
        read_file_date()
        keyboard_input = input("你要更改的内容:\n")
        if keyboard_input == "返回\n":
            return
        else:
            for key in dic_date:
                if keyboard_input == str(dic_date[key][0]):
                    print(
                        "\n你要更改的内容是:\t" + str(key) + "\t" + str(dic_date[key][0]) + "\t" + str(dic_date[key][1]) + "\n")
                    renew_part = input("\n你想修改成的内容是:\n")
                    if keyboard_input == "返回":
                        return
                    else:
                        print("\n确认你是否要把:\t" + dic_date[key][0] + "\t修改成\t" + renew_part + "\t?\n")
                        if input("\n确认:\n") == "是":
                            dic_date[key][0] = renew_part
                            list_time = list(dic_date)
                            list_value = list(dic_date.values())
                            txt_high = len(list_time)
                            for i in range(txt_high):
